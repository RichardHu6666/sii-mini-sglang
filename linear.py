...
class _LinearTPImpl(BaseOP):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.weight.dtype == torch.int8:
            weight_fp = self.weight.to(x.dtype) * getattr(self, "weight_scale", 1.0)
            bias_fp = self.bias.to(x.dtype) if self.bias is not None else None
...
class LinearOProj(_LinearTPImpl):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.weight.dtype == torch.int8:
            weight_fp = self.weight.to(x.dtype) * getattr(self, "weight_scale", 1.0)
            bias_fp = self.bias.to(x.dtype) if self.bias is not None else None
            y = F.linear(x, weight_fp, bias_fp)
        else:
            weight = self.weight.to(x.dtype) if self.weight.dtype != x.dtype else self.weight
            bias = self.bias.to(x.dtype) if self.bias is not None and self.bias.dtype != x.dtype else self.bias
...
class LinearRowParallel(_LinearTPImpl):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.weight.dtype == torch.int8:
            weight_fp = self.weight.to(x.dtype) * getattr(self, "weight_scale", 1.0)
            y = F.linear(x, weight_fp, self.bias)
        else:
