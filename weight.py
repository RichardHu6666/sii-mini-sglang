...
def load_weight(model_path: str, device: torch.device, quantization: str = "none") -> Iterator[Tuple[str, torch.Tensor]]:
    ...
    for file in tqdm(files, desc="Loading weights", disable=not tp_info.is_primary()):
        with safetensors.safe_open(file, framework="pt", device=str(device)) as f:
            for name in f.keys():
                ...
                # --- 新增的量化拦截函数 ---
                def yield_quantized(key, tensor):
                    # 只量化 Linear 层 (包含 proj, fc, dense)，跳过 MoE 专家层以保证兼容性
                    if quantization == "w8a16" and any(x in key for x in ["proj", "fc", "dense"]) and "expert" not in key:
                        # 按行计算 Scale (dim=-1)
                        scale = tensor.abs().max(dim=-1, keepdim=True)[0] / 127.0
                        scale = scale.clamp(min=1e-5)
                        # 量化为 Int8
                        weight_int8 = torch.round(tensor / scale).to(torch.int8)
                        yield key, weight_int8
                        yield key + "_scale", scale.to(tensor.dtype)
                    else:
                        yield key, tensor
                # --------------------------
                ...
                if config.is_moe and (expert_info := _get_expert_stack_info(out[0])) is not None:
                    ...
                    yield from yield_quantized(packed_key, torch.stack(experts, dim=0))
                else:  # Normal dense model
                    yield from yield_quantized(out[0], out[1])
